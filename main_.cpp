...
int main(int argc, char* argv[]) {
	...
	// ∂¡»°Õº∆¨
	// middleburry
	vector<string> objectList = { "tsukuba", "venus", "teddy", "cones" }; // cones, teddy, shopvac, Books°¢venus°¢tsukuba
	vector<string> leftNameList = { "scene1.row3.col3", "im2", "im2", "im2" };
	vector<string> rightNameList = {"scene1.row3.col4", "im6", "im6", "im6"};
	vector<string> dispNameList = { "truedisp.row3.col3", "disp2", "disp2", "disp2" };

	vector<float> disp_reduceCoeffList = {16, 8, 4, 4};
	vector<int> maxdispList = {15, 19, 59, 59};
	for (int i = 0; i < 4; i++)
	{
		...
		if (I1.empty() || I2.empty() || I1_c.empty() || I2_c.empty() || all_maskM.empty() || nonocc_maskM.empty() || disc_maskM.empty())
		{
		...
		StereoMatching::Parameters param(maxdispList[i]);
